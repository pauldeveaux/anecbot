...
async def claim_leaderboard_reset(db: aiosqlite.Connection, guild_id: int) -> bool:
    ...
    cursor = await db.execute(
        ...
        "WHERE guild_id = ? AND leaderboard_reset_in_progress = 0",
        (guild_id,),
    )
    await db.commit()
...
async def mark_leaderboard_published(db: aiosqlite.Connection, guild_id: int) -> None:
    """Record that the pre-reset standings message was sent."""
    await db.execute(
        "UPDATE guilds SET leaderboard_reset_published = 1 WHERE guild_id = ?",
        (guild_id,),
    )
    await db.commit()
