...
DM_GUIDE = (
    ...
    "- Cible du serveur (un joueur inscrit) ou cible personnalisée (nom libre + tes propres "
    "mauvais choix)\n"
    "- Après le texte, tu peux choisir d'ajouter une image (pièce jointe)\n\n"
    "**Voter**\n"
    "Quand une anecdote est publiée, réponds au QCM pour deviner à qui elle correspond, et "
    "note sa qualité de 1 à 5 — cette note donne des points bonus/malus à l'auteur.\n\n"
    "**Inscription**\n"
    "- `/leave [role]` — se désinscrire d'un rôle ou complètement\n\n"
    "Les commandes suivantes sont disponibles **sur le serveur** :\n"
    "- `/stats game` — statistiques du jeu\n"
    "- `/stats player [user]` — statistiques d'un joueur (toi-même si non précisé)\n"
    "- `/next` — prochains événements\n"
    "- `/leaderboard [kind]` — classement (points par défaut)\n\n"
    "**Règles**\n"
...
USER_GUIDE = (
    ...
    "à qui elle correspond, et note sa qualité de 1 à 5.\n\n"
    "**Inscription**\n"
    "- `/leave <role>` — se désinscrire d'un rôle ou complètement\n\n"
    "**Statistiques**\n"
    "- `/stats game` — voir les statistiques du jeu\n"
    "- `/stats player [user]` — voir les statistiques d'un joueur (toi-même si non précisé)\n"
    "- `/next` — voir les prochains événements prévus\n\n"
    "**Leaderboard**\n"
    "- `/leaderboard` — classement par points (par défaut)\n"
